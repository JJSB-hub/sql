...
SELECT s.customer_id, s.market_date, s.visit_log
FROM (
  SELECT DISTINCT customer_id, market_date, 
  DENSE_RANK() OVER (
    PARTITION BY customer_id
...
WHERE s.visit_log = 1
ORDER BY s.customer_id, s.market_date DESC;
...
SELECT market_date, total_sales, 'best' AS best_or_worst 
FROM ranked
WHERE best_rank = 1
UNION ALL
SELECT market_date, total_sales, 'worst' AS best_or_worst
FROM ranked
WHERE worst_rank = 1
...
SELECT vp.vendor_name, vp.product_name, SUM(5 * vp.price_per_qty) AS total_per_product
FROM (
  SELECT v.vendor_name, p.product_name, MAX(vi.original_price) AS price_per_qty
...
VALUES (
 16, 'Sweet Corn', 'Ear', 1, 'unit', CURRENT_TIMESTAMP
);
...
-- Determining the last quantity per product:

SELECT vi.product_id, vi.quantity
FROM vendor_inventory AS vi
WHERE vi.market_date = (
  SELECT MAX(v.market_date)
  FROM vendor_inventory AS v
  WHERE v.product_id = vi.product_id
);
